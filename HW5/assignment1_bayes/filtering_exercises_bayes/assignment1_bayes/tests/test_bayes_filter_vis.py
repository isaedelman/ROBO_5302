...
from filtering_exercises_bayes.assignment1_bayes.bayes_filter import (
    BayesFilter,
)
...
def test_bayes_filter_visualization():
    ...
    env = GridWorld(
        size=(10, 10), obstacle_positions=obstacles, sensor_noise_std=0.3
    )
    ...
    for _ in range(n_steps):
        ...
        action = np.random.randint(
            0, 3
        )  # Random action (0: forward, 1: turn right, 2: turn left)
        true_state, _, _ = env.step(action)
        true_state = list(
            true_state
        )  # Convert tuple to list to match existing code
        ...
        time.sleep(0.2)  # Pause to see the update
