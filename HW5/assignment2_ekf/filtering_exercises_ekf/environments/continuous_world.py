...
class ContinuousWorld:
    ...
    def __init__(
        ...
        self.obstacle_paths = [
            mpath.Path(obstacle) for obstacle in self.obstacles
        ]
        ...
        self.station = np.array(
            [8.0, 2.0]
        )  # ranging station (change as you like)
    ...
    def _get_velocity_scaling(self, position: np.ndarray) -> float:
        ...
        scale = 1.0 + 0.5 * np.sin(self.terrain_frequency * x) * np.sin(
            self.terrain_frequency * y
        )
        return np.clip(
            scale, 0.5, 1.5
        )  # Ensure scaling is between 0.5 and 1.5
