...
class EKFVisualizer:
    ...
    def _setup_plot(self):
        ...
        # Draw boundaries
        self.ax.plot(
            [0, self.env.size[0], self.env.size[0], 0, 0],
            [0, 0, self.env.size[1], self.env.size[1], 0],
            "k-",
            linewidth=2,
        )
    ...
    def update_plots(self, true_state=None):
        ...
        self.est_heading = self.ax.arrow(
            mean[0],
            mean[1],
            dx,
            dy,
        ...
        if true_state is not None:
            ...
            self.true_heading = self.ax.arrow(
                true_state[0],
                true_state[1],
                dx,
                dy,
        ...
        r_pred = self.env.range_to_station(
            self.filter.mu[:2]
        )  # predicted r from current est
        if self.est_range_circle is not None:
            self.est_range_circle.remove()
        self.est_range_circle = Circle(
            self.env.station, r_pred, fill=False, ec="b", ls="--", alpha=0.4
        )
