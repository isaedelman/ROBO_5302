...
class ExtendedKalmanFilter:
    ...
    def __init__(self, env, initial_state=None):
        ...
        self.min_variance = np.array(
            [0.01, 0.01, 0.01]
        )  # Minimum allowed variance
        self.max_variance = np.array(
            [2.0, 2.0, 1.0]
    ...
    def _motion_model(
        ...
        """
        Implement the nonlinear motion model for the unicycle robot.

        This method should compute the predicted next state based on the
        control inputs (forward velocity and angular velocity), using the
        robot's kinematic model and terrain-dependent velocity scaling.
        ...
        """
        # 1. Extract x, y, heading (theta) from input state
        x, y, theta = state

        # 2. Velocity scaling
        vel_scale = self.env._get_velocity_scaling(state[:2])

        # 3. Effective forward velocity
        effective_vel = forward_vel * vel_scale

        # 4. Speed-dependent turning factor
        ...
        # 6. Return predicted next state as numpy array
    ...
    def _motion_jacobian(
        ...
        """
        Implement the Jacobian of the motion model (F = ∂f/∂x).

        This method should compute the Jacobian of the nonlinear motion model
        with respect to the state vector [x, y, heading]. You will likely want
        to compute the partial derivatives by hand and check them before
        implementation.
        ...
        2. Compute the velocity scaling factor:
           ...
           and its spatial derivatives:
             dvs_dx, dvs_dy = partial derivatives of velocity scaling with
             respect to x and y.
        3. Compute the effective forward velocity:
             effective_vel = forward_vel * vel_scale
           This term appears in the partial derivatives of x and y with
           respect to heading.
        ...
        5. Build a 3×3 Jacobian matrix F initialized as identity (np.eye(3)).
           Then fill in its partial derivatives according to the unicycle
           model:
             - F[0,0], F[0,1] describe how x_next changes with x and y
             (through dvs_dx, dvs_dy).
             - F[0,2] = ∂x_next/∂θ ≈ -dt * effective_vel * sin(θ)
             - F[1,0], F[1,1] similarly affect y_next via sin(θ).
             - F[1,2] = ∂y_next/∂θ ≈ dt * effective_vel * cos(θ)
             - F[2,0], F[2,1] will include the speed_factor and dvs_dx, dvs_dy
             respectively.
        ...
        # 2. Velocity scaling and its derivatives
        vel_scale = self.env._get_velocity_scaling(state[:2])

        # Numerical approximation of derivatives
        delta = 1e-5
        # Use this function from environment to get velocity scaling
        # at perturbed positions
        dvs_dx = (
            self.env._get_velocity_scaling(np.array([x + delta, y]))
            - vel_scale
        ) / delta
        dvs_dy = (
            self.env._get_velocity_scaling(np.array([x, y + delta]))
            - vel_scale
        ) / delta
        ...
        # 5. Build Jacobian matrix F
        F = np.eye(3)

        # Partial derivatives for x_next
        F[0, 0] += dt * forward_vel * dvs_dx * np.cos(theta)
        F[0, 1] += dt * forward_vel * dvs_dy * np.cos(theta)
        F[0, 2] += -dt * effective_vel * np.sin(theta)

        # Partial derivatives for y_next
        F[1, 0] += dt * forward_vel * dvs_dx * np.sin(theta)
        F[1, 1] += dt * forward_vel * dvs_dy * np.sin(theta)
        F[1, 2] += dt * effective_vel * np.cos(theta)

        # Partial derivatives for heading_next
        # If the effective velocity is zero, heading does not change with
        # position.
        # So we only compute these terms if effective_vel != 0
        if effective_vel != 0:
            d_speed_dv = np.sign(effective_vel) / self.env.max_velocity
            d_speed_factor_dv = d_speed_dv if 0.5 < speed < 1.5 else 0.0
            F[2, 0] += (
                -angular_vel * d_speed_factor_dv * dvs_dx / (speed_factor**2)
            ) * dt
            F[2, 1] += (
                -angular_vel * d_speed_factor_dv * dvs_dy / (speed_factor**2)
            ) * dt
    ...
    def _measurement_jacobian(self, state: np.ndarray) -> np.ndarray:
        """
        Implement the Jacobian of the measurement model (H = ∂h/∂x).
        ...
        The measurement model is nonlinear:
            ...
            (z, the measurement, is the same as r, the range between the robot
            and station)
        ...
        1. Compute position differences between the robot and the station:
             ...
             (x_s and y_s can be found in self.env.station[0] and
             self.env.station[1])
        ...
        3. Compute the partial derivatives of the range with respect to
           each state variable using the chain rule:
             ∂r/∂x = ?   (compute these by differentiating the expression for
             range w.r.t. x and y)
             ∂r/∂y = ?
        ...
        4. Handle the special case where r is very close to zero (to avoid
           division by zero). In that case, return a zero matrix. 1e-9 is a
           good threshold.
        ...
        """
        # 1. Extract x, y, heading from state
        x, y, _ = state
        x_s, y_s = self.env.station

        # 2. Compute position differences
        dx = x - x_s
        dy = y - y_s
    ...
    def _constrain_covariance(self):
        ...
        self.covariance[diag_idx] = np.clip(
            self.covariance[diag_idx], self.min_variance, self.max_variance
        )
    ...
    def predict(self, forward_vel: float, angular_vel: float, dt: float):
        """
        Implement the Extended Kalman Filter prediction step.

        This method should implement the prediction step of the EKF using a
        nonlinear motion model
        that accounts for the robot's unicycle dynamics and terrain-dependent
        velocity scaling.
        ...
        2. Update state using nonlinear motion model:
           - Call self._motion_model(self.state, forward_vel, angular_vel, dt)
           and store result in self.state
        ...
        3. Compute Jacobian of motion model:
           - Call self._motion_jacobian(self.state, forward_vel,
           angular_vel, dt)
        ...
        6. Store state and covariance history:
           - Append copies of current state and covariance to
           self.state_history and self.covariance_history
        ...
        """
        # 1. Store control inputs
        self.last_forward_vel = forward_vel
        self.last_angular_vel = angular_vel
        self.last_dt = dt

        # 2. Update state using motion model
        self.state = self._motion_model(
            self.state, forward_vel, angular_vel, dt
        )
        self._normalize_heading()

        # 3. Compute motion Jacobian
    ...
    def update(self, measurements: np.ndarray):
        """
        Implement the Extended Kalman Filter update step.

        This method should implement the update step of the EKF using
        nonlinear range measurements
        from multiple beams. The measurement model accounts for hallway
        geometry and obstacles.

        Implementation steps:
        1. Get expected measurements:
           - Reshape actual measurement received from the measurements input
           to (1,)
           - Call self._measurement_model(self.state) to get expected
           measurement
        ...
        3. Compute Kalman gain:
           - Innovation covariance: S = H @ P @ H.T + R
                - P and R are stored as attributes of self (self.covariance
                and self.R)
           - Kalman gain: K = P @ H.T @ inv(S)

        4. Update state:
           - Compute innovation: y = measurements - expected_z
           - Compute state update: dx = K @ y, use .reshape(-1) to change (3,1)
           vector to
            (3,) for apply update step
           - Limit update magnitude if needed (based on
           self.max_position_update)
        ...
        6. Store updated state and covariance:
           - Append to state to self.state_history and covariance to
           self.covariance_history

        Parameters:
            measurements (np.array) : single distance measurement to ranging
            station
        """
        # 1. Get expected measurements
        ...
        # 4. Update state
        y = measurements - expected_z  # Innovation
        dx = K @ y
        dx = dx.reshape(-1)  # Reshape to (3,)

        # Limit update magnitude
        position_update_magnitude = np.linalg.norm(dx[:2])
        if position_update_magnitude > self.max_position_update:
            dx[:2] *= self.max_position_update / position_update_magnitude

        self.state = self.state + dx
        self._normalize_heading()

        # 5. Update covariance use Joseph form for numerical stability
    ...
    def get_velocity(self) -> np.ndarray:
        ...
        return np.array(
            [effective_vel * np.cos(heading), effective_vel * np.sin(heading)]
        )
