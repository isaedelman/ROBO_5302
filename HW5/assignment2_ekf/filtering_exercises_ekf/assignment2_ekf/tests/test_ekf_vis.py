...
from filtering_exercises_ekf.assignment2_ekf.extended_kalman_filter import (
...
def test_ekf_visualization():
    ...
    print(
        f"Ranging station located at: {env.station if hasattr(env, 'station') else '(not defined yet)'}"
    )
    ...
    try:
        for i in range(200):
            ...
            if i < 50:  # Move diagonally down-left
                ...
                if (
                    env.agent_pos[0] < 1.0
                    or env.agent_pos[0] > env.size[0] - 1.0
                    or env.agent_pos[1] < 1.0
                    or env.agent_pos[1] > env.size[1] - 1.0
                ):
                    angular_vel = 0.5
            ...
            true_state, _, collision = env.step(
                np.array([forward_vel, angular_vel])
            )
            ...
            if i % 50 == 0:
                ...
                print(
                    f"Measured range: {z:.3f}, Predicted: {z_pred[0]:.3f}, Innovation: {innovation:.3f}"
                )

            # Calculate errors
            pos_error = np.linalg.norm(true_state[:2] - ekf.mu[:2])
            heading_error = np.abs(
                (true_state[2] - ekf.mu[2] + np.pi) % (2 * np.pi) - np.pi
            )
            ...
            window_size = 10
            avg_pos_error = (
                np.mean(position_errors[-window_size:])
                if len(position_errors) > window_size
                else pos_error
            )
            avg_heading_error = (
                np.mean(heading_errors[-window_size:])
                if len(heading_errors) > window_size
                else heading_error
            )
            avg_innovation = (
                np.mean(measurement_innovations[-window_size:])
                if len(measurement_innovations) > window_size
                else measurement_innovations[-1]
            )
            ...
            if (
                i % 2 == 0
            ):  # Update display every other frame for smoother performance
                plt.draw()
