...
from filtering_exercises_particle_filter.assignment3_particle_filter.particle_filter import (
    ParticleFilter,
)


def plot_robot_and_particles(
    ax, true_state, particles, weights, landmarks, obstacles
):
    ...
    n_arrows = len(particles)
    arrow_indices = np.random.choice(
        len(particles), n_arrows, p=weights / np.sum(weights)
    )
    arrow_length = 0.3
...
def test_particle_filter_visualization():
    ...
    true_state = np.array(
        [env.agent_pos[0], env.agent_pos[1], env.agent_heading]
    )
    ...
    try:
        ...
        for t in range(100):  # 10 seconds with dt=0.1
            ...
            true_state = np.array(
                [next_state[0], next_state[1], env.agent_heading]
            )

            # Visualize
            plot_robot_and_particles(
                ax, true_state, pf.particles, pf.weights, [], env.obstacles
            )
            plt.draw()
            plt.pause(0.01)
