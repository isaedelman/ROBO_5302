...
This module implements various reinforcement learning algorithms for solving
gridworld problems using tabular methods. It includes value iteration,
deterministic policy iteration, and stochastic policy iteration approaches.
...
student_name = "Isaac Edelman"  # Set to your name
...
class TabularPolicy:
    ...
    This class implements a tabular policy and value function for
    reinforcement learning in discrete state/action spaces. It maintains
    mappings from states to values and from states to action probability
    distributions.

    Attributes:
        num_states (int): Total number of discrete states
        num_actions (int): Total number of possible actions
        state_ranges (np.ndarray):
        Array of [min, max) ranges for each state dimension
        _value_function (np.ndarray): Array mapping states to their values
        _policy (np.ndarray):
        Array mapping states to action probabilitydistributions
    """

    def __init__(
        self, n_states: int, state_ranges: np.ndarray, n_actions: int
    ) -> None:
        """
        ...
        # Create data structure to store array with probability of each action
        # for each state
        self._policy = np.random.uniform(
            0, 1, size=(n_states, self.num_actions)
        )

    def get_action(self, state: Union[int, np.ndarray]) -> int:
        """
        Sample an action from the policy's action distribution for the given
        state.
    ...
    def get_state_value(self, state: Union[int, np.ndarray]) -> float:
        ...
        if isinstance(state, int):
            ...
            return self._value_function[
                self.get_state_index_from_coordinates(state)
            ]
    ...
    def get_state_index_from_coordinates(self, state: np.ndarray) -> int:
        ...
        return (
            state[0] * (self.state_ranges[0][1] - self.state_ranges[0][0])
            + state[1]
        )
    ...
    def get_coordinates_from_state_index(self, state_idx: int) -> np.ndarray:
        ...
        Returns:
            A numpy array containing the (x,y) coordinates corresponding to
            the state index
        """
        return np.array(
            [
                state_idx
                // (self.state_ranges[0][1] - self.state_ranges[0][0]),
                state_idx
                % (self.state_ranges[0][1] - self.state_ranges[0][0]),
            ]
        )
    ...
    def get_value_function(self) -> np.ndarray:
        ...
        Returns:
            A numpy array representing the value function table,
            where each entry maps a state index to its value
        """
    ...
    def set_policy(self, state: int, action_prob_array: np.ndarray) -> None:
        ...
        Args:
            state: The state index
            action_prob_array: A numpy array containing probabilities for
            each action
        """
    ...
    def get_policy(self, state: Union[int, np.ndarray]) -> np.ndarray:
        ...
        Returns:
            A numpy array containing probabilities for each action in the
            given state
        """
        if isinstance(state, int):
...
class GridworldSolver:
    ...
    This class implements various policy computation methods for gridworld
    environments, including deterministic value iteration, stochastic policy
    iteration, and deterministic policy iteration.
    ...
    Attributes:
        ...
        solver (TabularPolicy):
        The policy object that stores computed policies and values
        performance_history (List[float]):
        History of cumulative rewards from policy evaluations
    """
    ...
    def __init__(
        ...
        assert policy_type in [
            ...
            "stochastic_pi",
            "deterministic_pi",
            "policy_iteration_exact_solve",
        ]
        self.env: Optional[gym.Env] = None
        self.env_name = ""
        self.init_environment(gridworld_map_number, noisy_transitions)
        self.theta = (
            1e-3  # parameter to determine when to stop policy evaluation
        )
    ...
    def compute_policy(self) -> None:
        ...
        This method selects and runs the appropriate policy computation
        algorithm based on the policy_type specified during initialization.
        """
    ...
    def solve(
        ...
        This method runs the current policy from a given start state
        (or default start state) and returns the cumulative reward and
    ...
    def plot_policy_curve(
        ...
        if filename is None:
            ...
            figures_dir = os.path.join(
                script_dir, "..", "..", "hw4_rl", "figures"
            )
            os.makedirs(figures_dir, exist_ok=True)
            filename = os.path.join(
                figures_dir, "gridworld_learning_curve.png"
            )
    ...
    def plot_policy(
        ...
        # Arrow directions
        # (assuming env.actions = [0:UP, 1:RIGHT, 2:DOWN, 3:LEFT])
        arrow_dict = {
    ...
    def _value_iteration(self) -> None:
        ...
        This method iteratively updates state values based on the Bellman
        optimality equation until convergence. The student needs to implement:
        ...
        The transition probabilities T and rewards R are pre-computed and
        stored in the T and R matrices respectively.
        """
        ...
        for _ in range(horizon):
            ...
            for s in range(unwrapped_env.num_states):
                ...
                for a in range(unwrapped_env.num_actions):
                    q_values[a] = np.sum(
                        T[s, a, :] * (R[s, a, :] + self.gamma * v_old)
                    )

                # Save best action value
                v_i[s] = np.max(q_values)
            ...
            for s in range(unwrapped_env.num_states):
                ...
                new_policy = np.zeros(unwrapped_env.num_actions)
                new_policy[best_action] = 1.0
                p_i[s] = new_policy

            # Record performance each iteration
            self.performance_history.append(np.mean(v_i))

            # Check convergence
            if np.max(np.abs(v_i - v_old)) < self.theta:
                break
    ...
    def _deterministic_policy_iteration(self) -> None:
        ...
        improvement steps, selecting the best action in each state
        deterministically. The student needs to implement:
        ...
        2. Policy Improvement: Update policy to be deterministic for best
        action:
           pi(s,a) = 1 if a = argmax_a Q(s,a), 0 otherwise
           where Q(s,a) = sum_s' T(s,a,s')[R(s,a,s') + gamma * V(s')]
        ...
        for k in range(horizon):
            ...
            for _ in range(horizon):
                ...
                for s in range(unwrapped_env.num_states):
                    # Find the deterministic action a for state s
                    a_star = np.argmax(p_i[s])
                    # Compute Bellman expectation for this action
                    v_new[s] = np.sum(
                        T[s, a_star, :] * (R[s, a_star, :] + self.gamma * v_i)
                    )
            ...
            policy_stable = True
            for s in range(unwrapped_env.num_states):
                old_a = np.argmax(p_i[s])
                q_values = np.zeros(unwrapped_env.num_actions)
                for a in range(unwrapped_env.num_actions):
                    # Bellman s = state, a = action, v_i = value function
                    # Value function is the expected return following policy pi
                    q_values[a] = np.sum(
                        T[s, a, :] * (R[s, a, :] + self.gamma * v_i)
                    )
    ...
    def exact_policy_evaluation(self) -> None:
        """
        Evaluate a deterministic policy exactly by solving:
        (I - gamma * P_pi) * V = R_pi
        Args:
        T: Transition matrix [S x A x S']
        R: Reward matrix [S x A x S']
        p_i: Policy matrix [S x A]
        ...
        # Build P_pi and R_pi
        P_pi = np.zeros((unwrapped_env.num_states, unwrapped_env.num_states))
        R_pi = np.zeros(unwrapped_env.num_states)

        for s in range(unwrapped_env.num_states):
            for a in range(unwrapped_env.num_actions):
                P_pi[s, :] += p_i[s, a] * T[s, a, :]
                R_pi[s] += p_i[s, a] * np.sum(T[s, a, :] * R[s, a, :])

        # Solve linear system
        I = np.eye(unwrapped_env.num_states)
...
if __name__ == "__main__":
    ...
    # pi world 0
    gw0_solver_pi_0 = GridworldSolver(
        policy_type="deterministic_pi", gridworld_map_number=0
    )

    for solver in [gw0_solver_pi_0]:
        start_time = time.time()
        solver.compute_policy()
        elapsed_time = time.time() - start_time
        print(
            "Computed Q1 Deterministic_PI Policy in %g seconds" % elapsed_time
        )

        solver.plot_policy_curve(solver.performance_history)

        gw0_solver_pi_0.plot_policy()

        gw0_solver_pi_0.plot_value_function(solver.solver.get_value_function())

    # pi world 1
    gw0_solver_pi_1 = GridworldSolver(
        policy_type="deterministic_pi", gridworld_map_number=1
    )

    for solver in [gw0_solver_pi_1]:
        start_time = time.time()
        solver.compute_policy()
        elapsed_time = time.time() - start_time
        print(
        ...
        gw0_solver_pi_1.plot_policy()

        gw0_solver_pi_1.plot_value_function(solver.solver.get_value_function())
    ...
    """# p_e world 0
    exact_p_e_0 = GridworldSolver(
        policy_type="policy_iteration_exact_solve", gridworld_map_number=0
    )

    for solver in [exact_p_e_0]:
        start_time = time.time()
        solver.compute_policy()
        print(f"Exact policy done in {time.time() - start_time:.4f} seconds")

        exact_p_e_0.plot_policy()

        exact_p_e_0.plot_value_function(solver.solver.get_value_function())

        solver.plot_policy_curve(solver.performance_history)

    # p_e world 1
    exact_p_e_1 = GridworldSolver(
        policy_type="policy_iteration_exact_solve", gridworld_map_number=1
    )

    for solver in [exact_p_e_1]:
        start_time = time.time()
        solver.compute_policy()
        print(f"Exact policy done in {time.time() - start_time:.4f} seconds")

        exact_p_e_1.plot_policy()

        exact_p_e_1.plot_value_function(solver.solver.get_value_function())
...
"""
    ...
    """# vi world 0
    gw0_solver_vi_0 = GridworldSolver(
        policy_type="deterministic_vi", gridworld_map_number=0
    )

    for solver in [gw0_solver_vi_0]:
        start_time = time.time()
        solver.compute_policy()
        elapsed_time = time.time() - start_time
        print("Computed Q2 Deterministic_VI Policy in %g seconds" % elapsed_time)

        solver.plot_policy_curve(solver.performance_history)

        gw0_solver_vi_0.plot_policy()

        gw0_solver_vi_0.plot_value_function(solver.solver.get_value_function())

    # vi world 1
    gw0_solver_vi_1 = GridworldSolver(
        policy_type="deterministic_vi", gridworld_map_number=1
    )

    for solver in [gw0_solver_vi_1]:
        start_time = time.time()
        solver.compute_policy()
        elapsed_time = time.time() - start_time
        print("Computed Q2 Deterministic_VI Policy in %g seconds" % elapsed_time)

        gw0_solver_vi_1.plot_policy()

        gw0_solver_vi_1.plot_value_function(solver.solver.get_value_function())
