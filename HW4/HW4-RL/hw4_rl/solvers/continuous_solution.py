...
class TabularPolicy:
    ...
    def __init__(
        self, num_bins_per_dim: int, num_dims: int, num_actions: int
    ) -> None:
        """
        ...
        self._policy = np.random.uniform(
            0, 1, size=(self.num_states, self.num_actions)
        )
...
class DiscretizedSolver:
    ...
    def add_transition(self, state_idx: int, action_idx: int) -> None:
        ...
        for i in range(n_samples):
            next_states = self.get_discrete_state_probabilities(
                next_states_list[i]
            )
            ...
            for next_state, prob in next_states:
                ...
                self.solver.set_transition(
                    state_idx, action_idx, next_state, new_prob
                )
                ...
                if new_prob > 0:
                    avg_reward = (
                        current_reward * current_prob
                        + reward * prob / n_samples
                    ) / new_prob
                else:
                    avg_reward = reward
                self.solver.set_reward(
                    state_idx, action_idx, next_state, avg_reward
                )

    def get_discrete_state_probabilities(
        self, continuous_state: np.ndarray
    ...
    def _value_iteration(
        self, max_iterations: int, min_iter: int, eval_sample_size: int
    ) -> None:
        """
        ...
        for i in range(max_iterations):
            ...
            reward, steps = self.solve(
                max_steps=200, sample_size=eval_sample_size
            )
    ...
    def _deterministic_policy_iteration(
        ...
        policy[
            ...
            np.random.randint(
                0, self.solver.num_actions, size=self.solver.num_states
            ),
        ] = 1.0
        value_function = np.zeros(self.solver.num_states)
        ...
        for k in range(max_iterations):
            ...
            # Policy evaluation
            policy_T = np.sum(
                self.solver._transition_function * policy[:, :, np.newaxis],
                axis=1,
            )
            policy_R = np.sum(
        ...
        print(
            f"Final policy performance: Reward of {reward:.2f} after {steps} steps."
        )
    ...
    def solve(
        ...
        for _ in range(sample_size):
            ...
            while not done and num_steps < max_steps:
                ...
                if self._mode == "nn":
                    discrete_state = self.get_discrete_state_probabilities(
                        state
                    )[0][0]
                    action = self.solver.get_action(discrete_state)
                else:
                    discrete_states = self.get_discrete_state_probabilities(
                        state
                    )
                    states, probs = np.array(discrete_states).T
                    action = self.solver.get_action(
                        int(states[np.argmax(probs)])
                    )
    ...
    def get_state_index_from_coordinates(
        self, continuous_state: np.ndarray
    ) -> int:
        """
        ...
        pos_bin = int(
            (continuous_state[0] - self.state_lower_bound[0]) / bin_sizes[0]
        )
        vel_bin = int(
            (continuous_state[1] - self.state_lower_bound[1]) / bin_sizes[1]
        )
    ...
    def plot_value_function(
        ...
        if filename is None:
            ...
            filename = os.path.join(
                figures_dir,
                f"{self.env_name}_{self._mode}_{self._num_bins}.png",
            )
    ...
    for n_bins in bin_sizes:
        ...
        for policy_type, label, temp in algorithms:
            ...
            solver.plot_value_function(
                ...
                os.path.join(
                    figures_dir,
                    f"mountaincar_{policy_type}_value_{n_bins}.png",
                ),
            )
            solver.plot_policy(
                os.path.join(
                    figures_dir,
                    f"mountaincar_{policy_type}_policy_{n_bins}.png",
                )
            )
        ...
        plot_policy_curves(
            ...
            bin_labels,
            os.path.join(
                figures_dir, f"mountaincar_learning_curves_{n_bins}_bins.png"
            ),
        )
