...
class GridworldEnv(gym.Env):
    ...
    def __init__(
        self, map_file: str = "plan0.txt", transition_noise: float = 0
    ) -> None:
        """
        ...
        self.agent_start_state = self._get_agent_start_state(
            self.start_grid_map
        )
    ...
    def step(
        self, action: int
    ) -> Tuple[np.ndarray, float, bool, bool, Dict[str, bool]]:
        """
    ...
    def get_state_ranges(self) -> np.ndarray:
        ...
        return np.array(
            [
                [0, self.start_grid_map.shape[0]],
                [0, self.start_grid_map.shape[1]],
            ]
        )
    ...
    def T(
        self, state: State, action: int, next_state: Optional[State] = None
    ) -> TransitionList:
        """
        ...
        if self.can_move(state, action, target_next_state):
            possible_next_states = [
                (1.0 - self._transition_noise, target_next_state)
            ]
        else:
        ...
        if next_state is not None:
            ...
            probability_mass_next_state = sum(
                prob
                for prob, state in possible_next_states
