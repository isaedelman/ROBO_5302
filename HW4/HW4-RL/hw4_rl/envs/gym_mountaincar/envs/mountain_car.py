...
This module implements the Mountain Car environment as described in
Andrew Moore's PhD Thesis (1990).
...
class MountainCarEnv(gym.Env):
    ...
    def step(
        ...
        velocity += (action - 1) * self.force + math.cos(3 * position) * (
            -self.gravity
        )
        ...
        # Stop sim if max steps reached
        ep_len = self.step_count >= 200
        print(
            "Reach Goal?: ",
            done,
            "Step Count: ",
        ...
        return self.state, reward, done, ep_len, {}
    ...
    def _height(
        self, xs: Union[float, np.ndarray]
    ...
    def render(self, mode: str = "human") -> Union[np.ndarray, bool]:
        ...
        ys = self._height(xs)
        # Flip rightside up bc it was upside down
        xys = list(
            zip((xs - self.min_position) * scale, screen_height - ys * scale)
        )

        pygame.draw.aalines(
            self.surf, points=xys, closed=False, color=(0, 0, 0)
        )
